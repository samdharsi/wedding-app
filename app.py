...
DB_PATH = os.environ.get("DB_PATH", "wedding.db")
...
def home():
    ...
    for e in upcoming:
        ...
        <tr>
          ...
          <td class="muted">{e['assigned_to'] or ''}</td>
          <td class="muted">{e['notes'] or ''}</td>
        </tr>
        """
    ...
    body += """
      ...
      <div class="muted small" style="margin-top:8px;">
        Tip: Admins can update status as work progresses.
      </div>
    </div>
    """
...
def login():
    ...
    <div class="card">
      <h2>Login</h2>
      <p class="muted">Use demo accounts (change PINs later).</p>
      <form method="post">
        <label>Username</label>
...
def guests():
    ...
    def guest_table(title, items):
        ...
        """
        for g in items:
            visited = "✅" if g["visited"] else "—"
            stay = "Yes" if g["stay_required"] else "No"
            html += f"""
            <tr>
              <td><b>{g['name']}</b><div class="muted small">{g['phone'] or ''}</div></td>
              <td class="muted">{g['relation'] or ''}</td>
              <td>{stay}</td>
              <td class="muted">{g['room_no'] or ''}</td>
              <td>{visited}</td>
              <td class="right">
...
def travel():
    ...
    if request.method == "POST":
        ...
        # Upsert style: if travel exists for guest, update; else insert
    ...
    guests = db_query("SELECT * FROM guests ORDER BY side ASC, name ASC")
    ...
    """
    for g in guests:
        t = travel_map.get(g["id"])
        if t:
            arrival = f"{t['arrival_date'] or ''} {t['arrival_time'] or ''} ({t['mode'] or ''}) {t['ref_no'] or ''}"
            pickup = "Yes" if t["pickup_required"] else "No"
            stay = f"{t['checkin_date'] or ''} → {t['checkout_date'] or ''}"
        ...
        body += f"""
        <tr>
          <td><b>{g['name']}</b><div class="muted small">{g['side']}</div></td>
    ...
    if is_admin(role):
        opts = "".join([f"<option>{s}</option>" for s in STATUS_OPTIONS])
        guest_opts = "".join([f"<option value='{g['id']}'>{g['name']} ({g['side']})</option>" for g in guests])
...
def venue_rooms():
    ...
    for r in rows:
        body += f"""
        <tr>
          <td><b>{r['room_no'] or ''}</b></td>
          <td class="muted">{r['guest_name'] or ''}</td>
          <td class="muted">{r['checkin'] or ''}</td>
          <td class="muted">{r['checkout'] or ''}</td>
          <td><span class="tag">{r['status']}</span></td>
          <td class="muted">{r['assigned_to'] or ''}</td>
...
def vendors():
    ...
    """
    for v in rows:
        contact = f"{v['contact_person'] or ''} {v['phone'] or ''}".strip()
        body += f"""
        <tr>
          <td><b>{v['category']}</b></td>
          <td class="muted">{v['vendor_name'] or ''}</td>
          <td class="muted">{contact}</td>
          <td><span class="tag">{v['status']}</span></td>
          <td class="muted">{v['assigned_to'] or ''}</td>
